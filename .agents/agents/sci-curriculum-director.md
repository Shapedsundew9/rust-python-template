...
2. **Three moves: exploit, mutate, ablate.**
   - **Exploit**: Narrow the parameter space around a promising region.
   - **Mutate**: Alter a structural primitive (topology, update rule, conservation mechanism).
   - **Ablate**: Remove a component to test its necessity.
