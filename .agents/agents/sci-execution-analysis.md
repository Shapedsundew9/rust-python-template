...
18. **Shared Component Reusability & Zero Boilerplate Duplication**: Prior completed experiment packages are strictly read-only and preserved for historical provenance. However, when shared domain libraries, core simulation primitives, mathematical kernels, or common telemetry/statistical utilities exist in the workspace (as defined in repository guidelines), new experiment packages MUST import and build upon them rather than re-implementing or copying identical utility code, PRNGs, baseline substrate engines, or statistical algorithms. The agent implements only the novel delta (the specific mechanism, topological configuration, parameter space, or protocol logic under evaluation) in the isolated experiment package.
