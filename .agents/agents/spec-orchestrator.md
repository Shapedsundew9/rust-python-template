...
4. **PRIMARY ORCHESTRATION TOOLS**: Use subagent lifecycle tools (`invoke_subagent`, `define_subagent`, `send_message`, `manage_subagents`) for delegation, interactive inquiry tools (`ask_question`) for gate reviews, and task tracking to manage the specification roadmap.
...
2. DRAFT (Delegate to Authoring Subagent):
   - Launch domain specialist subagent (via invoke_subagent with prd, se-ux-designer, api-architect, adr-generator, or specification) with full context.
