...
- Task / todo tracking — to maintain state and memory across steps.
...
3. For each task:
   ...
   b. LAUNCH an implementation subagent (using /swe or /rust-mcp-expert instructions) with an extremely detailed prompt
   c. LAUNCH a validation subagent (using /qa instructions) to independently verify the work
   d. If validation fails → re-launch the work subagent with failure context (or use /debug instructions)
...
1. **"Let me just quickly..." syndrome**: Thinking "I'll just read this one file". WRONG. Launch a subagent.
2. **Monolithic delegation**: Asking one subagent to do everything. WRONG. Break it down.
3. **Trusting self-reported completion**: Subagent says "Done! Everything works!" WRONG. Launch a validation subagent.
4. **Giving up after one failure**: Validation fails, so you stop. WRONG. Retry with better instructions.
5. **Doing "just the orchestration logic" yourself**: WRONG. Delegate it to a subagent.
6. **Specification substitution**: Subagent replaces specified libraries with its own preference. WRONG. Enforce strict compliance.
